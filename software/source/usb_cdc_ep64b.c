/********************************************************************
 ...
 (c) 2012, Thomas Fischl <tfischl@gmx.de>
     2013, Jürgen Liegner <juergen@liegner.de> expand bulk send buffer 
           to 0x40 bytes   
 Device: PIC18F14K50
...
// jl groesseren Buffer zum Bulk-Senden
#define EP_BUFFERSIZE_TX 0x40
...
/*Endpoint 1 Descriptor*/
        ...
        0x02,   /* bmAttributes: Bulk */
        EP_BUFFERSIZE_TX,  // jl /* wMaxPacketSize: */
        0x00,
        0x00    /* bInterval: ignore for Bulk transfer */
};
...
// die Groesse gabes gerade noch her
#define TXBUFFER_SIZE     130
...
volatile unsigned char ep1in_buffer[EP_BUFFERSIZE_TX] @ 0x2A8;  // jl ans Ende verlegt
...
void usb_txprocess() {
    ...
    // jl
    if (count > EP_BUFFERSIZE_TX - 1) count = EP_BUFFERSIZE_TX - 1;
...
void usb_init() {
    ...
    ep[1].in.stat = 0x40;
    ep[1].in.cnt = EP_BUFFERSIZE_TX;  // jl
    ep[1].in.adrl = 0xA8;              // jl
    ep[1].in.adrh = 0x02;
